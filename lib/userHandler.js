...
obj.verifyToken = async (token, socket) => {
    ...
    let userData = await conn.query("SELECT t1.name,'https://121directory.com/data/profile.jpg' as image,'" + date + "' as last_seen,t1." + tokenColumn + ",t1.id,t1.email from " + userTableName + " as t1 WHERE t1." + tokenColumn + "='" + token + "' LIMIT 1");
